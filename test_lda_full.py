...
def test_lda_full_pipeline():
    ...
    try:
        ...
        for i, reg in enumerate(top_registrants, 1):
            print(f"      {i}. {reg['name']} - ${reg['total_amount']:,} ({reg['filing_count']} filings)")
        ...
        for i, client in enumerate(top_clients, 1):
            print(f"      {i}. {client['name']} - ${client['total_amount']:,} ({client['filing_count']} filings)")
        ...
        for issue in issues:
            print(f"      • {issue['code']}: {issue['filing_count']} filings (${issue['total_amount']:,})")
        ...
        if "error" not in entity_result:
            ...
            print(f"      Total: ${entity_result['total_amount']:,} ({entity_result['filing_count']} filings)")
        else:
        ...
        help_result = real_slack_app._handle_lda_subcommands(["help"], "test_channel", "test_user")
        print("   ✅ LDA help command works")
        
        # Test top registrants command
        top_result = real_slack_app._handle_lda_subcommands(["top", "registrants", "n=3"], "test_channel", "test_user")
...
def test_smoke_tests():
    ...
    try:
        ...
        with db_manager.get_connection() as conn:
            ...
            issue_count = conn.execute("SELECT COUNT(*) FROM filing_issue").fetchone()[0]
        
        print(f"   ✅ Row counts: {filing_count} filings, {entity_count} entities, {issue_count} filing-issues")
