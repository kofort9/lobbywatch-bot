...
class SignalsDatabase:
    ...
    def _ensure_signals_schema(self):
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_signal_timestamp ON signal_event(timestamp)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_signal_source ON signal_event(source)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_signal_priority ON signal_event(priority_score)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_alias_text ON entity_alias(alias_text)")
    ...
    def get_recent_signals(self, hours_back: int = 24, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for row in cursor.fetchall():
            ...
            # Parse JSON fields
            try:
                signal['issue_codes'] = eval(signal['issue_codes']) if signal['issue_codes'] else []
                signal['metric_json'] = eval(signal['metric_json']) if signal['metric_json'] else {}
            except:
                signal['issue_codes'] = []
                signal['metric_json'] = {}
    ...
    def get_signals_by_issue(self, issue_code: str, hours_back: int = 24) -> List[Dict[str, Any]]:
    ...
    def add_entity_alias(self, entity_id: int, alias_text: str, source: str, confidence: float = 1.0) -> bool:
