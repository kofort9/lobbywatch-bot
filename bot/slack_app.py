...
class SlackApp:
    ...
    def handle_slash_command(self, command_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        logger.info(f"Slash command: {command} {text} in channel {channel_id} by user {user_id}")
    ...
    def _handle_watchlist_command(self, text: str, channel_id: str, user_id: str) -> Dict[str, str]:
    ...
    def _handle_watchlist_list(self, channel_id: str) -> Dict[str, str]:
        ...
        for entity_type, items in by_type.items():
            ...
            lines.append(f"\n*{type_name}:*")
            for item in items:
                score_text = f" ({item['fuzzy_score']:.0f}% match)" if item['fuzzy_score'] < 100 else ""
    ...
    def _handle_watchlist_add(self, search_term: str, channel_id: str, user_id: str) -> Dict[str, str]:
    ...
    def _handle_watchlist_remove(self, search_term: str, channel_id: str) -> Dict[str, str]:
    ...
    def _handle_threshold_command(self, text: str, channel_id: str, user_id: str) -> Dict[str, str]:
        ...
        if threshold_type == 'filings':
            self.db_manager.update_channel_setting(channel_id, 'threshold_filings', value)
            return {
                'response_type': 'in_channel',
                'text': f"✅ Mini-digest filing threshold set to **{value}**."
            }
        elif threshold_type == 'amount':
            self.db_manager.update_channel_setting(channel_id, 'threshold_amount', value)
            return {
                'response_type': 'in_channel',
    ...
    def _handle_summary_command(self, text: str, channel_id: str, user_id: str) -> Dict[str, str]:
        ...
        self.db_manager.update_channel_setting(channel_id, 'show_descriptions', show_descriptions)
    ...
    def _handle_lobbylens_command(self, text: str, channel_id: str, user_id: str) -> Dict[str, str]:
        """Handle /lobbylens command for manual digest generation."""
        if text.lower() == 'digest':
            # Generate manual digest
            try:
                digest = self.digest_computer.compute_enhanced_digest(channel_id, 'daily')
    ...
    def handle_message_event(self, event_data: Dict[str, Any]) -> Optional[Dict[str, str]]:
        ...
        if not confirmation or confirmation['channel_id'] != channel_id or confirmation['user_id'] != user_id:
            return None
