...
class DatabaseManager:
        ...
        with self.get_connection() as conn:
            ...
            if not result:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (
                    channel_id, defaults['threshold_filings'], 
                    defaults['threshold_amount'], defaults['show_descriptions'],
                    defaults['created_at']
                ))
    ...
    def add_to_watchlist(self, channel_id: str, entity_type: str, watch_name: str, 
                        display_name: str = None, entity_id: int = None, 
                        fuzzy_score: float = 1.0) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (channel_id, entity_type, entity_id, watch_name, 
                      display_name or watch_name, fuzzy_score))
                return True
    ...
    def record_digest_run(self, channel_id: str, run_type: str, 
                         filings_count: int = 0, last_filing_time: str = None,
        ...
        with self.get_connection() as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                channel_id, run_type, datetime.now().isoformat(),
                filings_count, last_filing_time, digest_content
            ))
    
    def get_last_digest_run(self, channel_id: str, run_type: str = None) -> Optional[Dict[str, Any]]:
    ...
    def add_entity_alias(self, alias_name: str, canonical_name: str, 
        ...
        with self.get_connection() as conn:
            ...
            """, (
                alias_name.lower(), canonical_name, entity_type, entity_id, 
                confidence_score, datetime.now().isoformat(),
                alias_name.lower(), entity_type
            ))
    
    def find_entity_alias(self, alias_name: str, entity_type: str = None) -> Optional[Dict[str, Any]]:
