...
class DigestComputer:
    ...
    def _get_new_filings(self, conn: sqlite3.Connection, since: datetime, limit: int = 10) -> List[sqlite3.Row]:
    ...
    def _get_top_registrants(self, conn: sqlite3.Connection, since: datetime, limit: int = 5) -> List[sqlite3.Row]:
    ...
    def _get_issue_surges(self, conn: sqlite3.Connection, week_start: datetime, prev_week_start: datetime, limit: int = 5) -> List[sqlite3.Row]:
    ...
    def compute_digest(self) -> str:
        ...
        if top_registrants:
            ...
            for reg in top_registrants:
                ...
                lines.append(f"• {name}: {amount} ({count} filing{'s' if count != 1 else ''})")
        else:
        ...
        if issue_surges:
            ...
            for issue in issue_surges:
                ...
                lines.append(f"• {code}: {current} filings (prev {previous}) {pct_change}")
        else:
