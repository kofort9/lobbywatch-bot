...
class EnhancedDigestComputer:
    ...
    def _is_watchlist_match(self, filing: sqlite3.Row, watchlist_entities: Dict[str, Set[int]], 
                           filing_issues: List[int]) -> List[str]:
    ...
    def _format_filing_entry(self, filing: sqlite3.Row, is_watchlist: bool = False, 
    ...
    def _get_enhanced_new_filings(self, conn: sqlite3.Connection, since: datetime, 
                                 channel_id: str, limit: int = 10) -> Tuple[List[sqlite3.Row], List[bool]]:
        ...
        for filing in filings:
            ...
            matches = self._is_watchlist_match(filing, watchlist_entities, filing_issues)
            watchlist_statuses.append(bool(matches))
        
        return filings, watchlist_statuses
    
    def _get_enhanced_top_registrants(self, conn: sqlite3.Connection, since: datetime, 
                                    channel_id: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    def _get_enhanced_issue_activity(self, conn: sqlite3.Connection, week_start: datetime, 
                                   prev_week_start: datetime, channel_id: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    def compute_enhanced_digest(self, channel_id: str, digest_type: str = 'daily') -> str:
        ...
        # New filings section
        if new_filings:
            time_desc = f"since {since.strftime('%H:%M')}" if digest_type == 'mini' else "last 24h"
            ...
            for filing in watchlist_filings:
                entry = self._format_filing_entry(filing, is_watchlist=True, 
            ...
            for filing in regular_filings[:display_count]:
                entry = self._format_filing_entry(filing, is_watchlist=False,
                                                show_description=settings['show_descriptions'])
                lines.append(entry)
            
            # Show total count if truncated
            total_remaining = len(new_filings) - len(watchlist_filings) - len(regular_filings[:display_count])
        ...
        if digest_type == 'mini' and len(new_filings) < 5:
            ...
            if top_registrants and digest_type == 'daily':
                ...
                for reg in top_registrants:
                    name_display = f"**{reg['name']}**" if reg['is_watchlist'] else reg['name']
            ...
            if issue_activity and digest_type == 'daily':
                ...
                for issue in issue_activity:
                    code_display = f"**{issue['code']}**" if issue['is_watchlist'] else issue['code']
                    ...
                    lines.append(f"• {full_name}: {current} filings (prev {previous}) {pct_change}")
        ...
        result = "\n".join(lines)
        logger.info(f"Generated {digest_type} digest with {len(lines)} lines for channel {channel_id}")
