...
class SignalV2:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SignalV2':
        ...
        metric_json = json.loads(data.get('metric_json', '{}')) if data.get('metric_json') else None
...
class SignalsRulesEngine:
    ...
    def _determine_urgency(self, signal: SignalV2) -> Urgency:
        ...
        if signal.signal_type == SignalType.DOCKET and signal.comment_count and signal.comment_count > 0:
            medium_urgency_conditions.append(True)
        
        # Bill committee referral
        if signal.signal_type == SignalType.BILL and signal.action_type == 'committee_referral':
            medium_urgency_conditions.append(True)
    ...
    def _calculate_priority_score(self, signal: SignalV2) -> float:
        ...
        if signal.metric_json and 'comments_24h_delta_pct' in signal.metric_json:
            ...
            surge_modifier = min(2.0, max(0, (delta_pct / 100) ** 0.5))  # log2 approximation
        ...
        final_score = (base_score + urgency_modifier + surge_modifier + 
                      deadline_modifier + watchlist_modifier + stale_penalty)
    ...
    def _check_watchlist_hit(self, signal: SignalV2) -> bool:
        ...
        text_to_search = f"{signal.title} {signal.summary} {signal.agency or ''}".lower()
    ...
    def group_dockets(self, signals: List[SignalV2]) -> Dict[str, List[SignalV2]]:
        ...
        for signal in signals:
            if signal.source == 'regulations_gov':
                ...
                docket_id = signal.stable_id.split('-')[0] if '-' in signal.stable_id else signal.stable_id
