...
class TestFixturesV2:
    ...
    def get_fixture_d_character_budget_stress() -> List[SignalV2]:
        ...
        for i in range(45):
            signal = SignalV2(
                source='congress' if i % 4 == 0 else 'federal_register' if i % 4 == 1 else 'regulations_gov',
                ...
                issue_codes=['TEC', 'HCR', 'ENV'] if i % 3 == 0 else ['FIN'] if i % 3 == 1 else ['ENE'],
...
class TestValidator:
    ...
    def validate_digest_format(self, digest_text: str) -> bool:
        ...
        required_sections = ['Watchlist Alerts', 'What Changed', 'Industry Snapshots', 
                           'Deadlines', 'Docket Surges', 'New Bills & Actions']
    ...
    def validate_section_limits(self, digest_text: str) -> bool:
        ...
        for section, count in section_counts.items():
            if count > limits.get(section, 0):
                self.errors.append(f"Section {section} exceeds limit: {count} > {limits[section]}")
    ...
    def validate_mobile_formatting(self, digest_text: str) -> bool:
        ...
        long_title_lines = [line for line in lines if len(line) > 80 and line.startswith('•')]
        if long_title_lines:
            self.warnings.append("Found long title lines that may need mobile formatting")
