...
class SignalsDatabaseV2:
    ...
    def _ensure_schema(self):
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_source ON signals_v2(source)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_timestamp ON signals_v2(timestamp)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_priority ON signals_v2(priority_score)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_urgency ON signals_v2(urgency)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_industry ON signals_v2(industry_tag)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_watchlist ON signals_v2(watchlist_hit)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_bill_id ON signals_v2(bill_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_signals_stable_id ON signals_v2(stable_id)")
    ...
    def get_industry_signals(self, industry: str, limit: int = 2) -> List[SignalV2]:
        ...
        cur.execute("""
            ...
            ORDER BY priority_score DESC
            LIMIT ?
        """, (industry, (datetime.now(timezone.utc) - timedelta(days=7)).isoformat(), limit))
    ...
    def add_watchlist_item(self, channel_id: str, entity_name: str, entity_type: str = 'entity') -> bool:
