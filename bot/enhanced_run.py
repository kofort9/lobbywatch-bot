...
def fetch_data() -> tuple[int, int]:
    ...
    if settings.opensecrets_api_key:
        try:
            ...
            logger.warning("lobbywatch.sources.opensecrets not available")
            failed += 1
    ...
    if settings.propublica_api_key:
        try:
            ...
            logger.warning("lobbywatch.sources.propublica not available")
            failed += 1
        except Exception as e:
...
def get_configured_channels() -> List[str]:
    ...
    # For now, use environment variable. In production, this could come from database
...
def run_scheduled_digests(db_manager: DatabaseManager, slack_app: SlackApp, 
                         digest_type: str = 'daily') -> Dict[str, bool]:
        ...
        try:
            ...
            if digest_type == 'mini':
                ...
                if not digest_computer.should_send_mini_digest(channel_id):
                    logger.info(f"Skipping mini digest for {channel_id} - threshold not met")
...
def main(mode: str, channel: str, dry_run: bool, skip_fetch: bool, 
    ...
    if settings.dry_run:
        ...
        # Generate sample digest for first configured channel or specified channel
        test_channels = [channel] if channel else get_configured_channels()[:1]
        
        if test_channels:
            try:
                digest_computer = EnhancedDigestComputer(db_manager)
                digest_text = digest_computer.compute_enhanced_digest(test_channels[0], mode)
