...
class FuzzyMatcher:
    ...
    def find_entity_matches(self, search_term: str, entity_type: str = None, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if not entity_type or entity_type == 'issue':
            ...
            for issue in issues:
                ...
                desc_score = self.similarity_score(search_term, issue['description'] or '')
    ...
    def categorize_matches(self, matches: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def create_confirmation_message(self, search_term: str, categorized_matches: Dict[str, List[Dict[str, Any]]]) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        if exact:
            if len(exact) == 1:
                ...
                message += "\nReply with number (1-{}), 'all', or 'q' to cancel.".format(len(candidates))
        ...
        if low_confidence:
            ...
            for i, match in enumerate(candidates, 1):
                message += f"{i}) {match['name']} ({match['score']:.0f}% match)\n"
            message += "\nReply with number (1-{}) or 'q' to cancel.".format(len(candidates))
            return message, candidates
        
        # No matches at all
        return f"No matches found for '{search_term}'. Try a more complete name (e.g., 'Alphabet Inc.' instead of 'Google').", []
...
class MatchingService:
    ...
    def process_watchlist_add(self, channel_id: str, search_term: str) -> Dict[str, Any]:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            message, candidates = matcher.create_confirmation_message(search_term, categorized)
    ...
    def process_confirmation_response(self, channel_id: str, search_term: str, 
