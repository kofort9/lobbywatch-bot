...
class SignalsRulesEngine:
    ...
    def __init__(self, watchlist: Optional[List[str]] = None):
        ...
        self.issue_mappings = {
            ...
            "TEC": [
                ...
                "cryptocurrency", "cybersecurity", "data privacy", "broadband",
                "5g", "internet", "telecommunications", "software", "cloud computing"
            ],
            
            # Healthcare
            "HCR": [
                "healthcare", "health care", "medical", "medicare", "medicaid",
                "pharmaceutical", "drug", "fda", "clinical trial", "public health"
            ],
            
            # Defense
            "DEF": [
                "defense", "military", "pentagon", "national security",
                "homeland security", "veterans", "armed forces"
            ],
            
            # Finance
            "FIN": [
                "banking", "financial", "securities", "investment", "credit",
                "lending", "mortgage", "insurance", "fintech"
            ],
            
            # Environment
            "ENV": [
                "environment", "climate", "epa", "pollution", "clean air",
                "water quality", "renewable energy", "sustainability"
            ],
            ...
            # Transportation
            "TRA": [
                "transportation", "highway", "aviation", "railroad", "shipping",
                "autonomous vehicle", "infrastructure"
            ],
            
            # Energy
            "FUE": [
                "energy", "oil", "gas", "coal", "nuclear", "renewable",
                "electricity", "grid", "pipeline"
            ],
            
            # Agriculture
            "AGR": [
                "agriculture", "farm", "food", "crop", "livestock",
                "rural", "usda"
            ]
        }
        ...
        self.type_patterns = {
            ...
            SignalType.PROPOSED_RULE: ["proposed rule", "notice of proposed rulemaking", "nprm"],
        ...
        self.urgency_keywords = {
            Urgency.CRITICAL: [
                "emergency", "immediate", "urgent", "critical", "national security",
                "public health emergency", "safety recall"
            ],
            Urgency.HIGH: [
                "final rule", "deadline", "comment period", "hearing", "markup",
                "significant", "major"
            ],
            Urgency.MEDIUM: [
                "proposed rule", "notice", "guidance", "report", "study"
            ]
        }
    ...
    def _calculate_priority_score(self, signal: SignalV2) -> float:
        ...
        hours_old = (datetime.now(timezone.utc) - signal.timestamp).total_seconds() / 3600
        if hours_old < 24:
...
class LegacySignalsProcessor:
    ...
    def __init__(self):
        import logging
        logging.warning("Using legacy V1 SignalsProcessor. Consider upgrading to SignalsRulesEngine.")
    
    def process_signals(self, signals: List[Dict]) -> List[Dict]:
        """Legacy signal processing (deprecated)."""
        import logging
        logging.warning("Legacy process_signals called. Use V2 SignalsRulesEngine instead.")
        return signals
