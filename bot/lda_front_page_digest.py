...
from .lda_issue_codes import get_issue_description
...
class LDAFrontPageDigest:
    ...
    def generate_digest(self, channel_id: str, quarter: Optional[str] = None) -> str:
        ...
        try:
            ...
            header = self._generate_header_narrative(
                year, q, prev_year, prev_q
            )
            ...
            if overflow_count > 0:
                digest_lines.append(f"+{overflow_count} more in thread · /lobbylens lda help · Updated {current_time}")
            else:
    ...
    def _get_new_amended_since_last_run(
        ...
        with self.db_manager.get_connection() as conn:
            # Build the query
            where_conditions = [f"f.year = ? AND f.quarter = ?"]
        ...
        if len(lines) < max_lines:
            with self.db_manager.get_connection() as conn:
                cursor = conn.execute("""
                    ...
                    ORDER BY total DESC
                    LIMIT 3
                """, (year, f"{year}Q{quarter}", year, year, f"{year}Q{quarter}", year - 2))
    ...
    def _get_largest_single_filings(
        ...
        with self.db_manager.get_connection() as conn:
            placeholders = ','.join(['?' for _ in exclude_uids]) if exclude_uids else "''"
            exclude_clause = f"AND f.filing_uid NOT IN ({placeholders})" if exclude_uids else ""
    ...
    def _generate_header_narrative(
        ...
        with self.db_manager.get_connection() as conn:
            ...
            top_reg_str = f"{top_reg['name']} ({format_amount(top_reg['total'])})" if top_reg else "—"
            ...
            top_issue_str = f"{top_issue['code']} ({format_amount(top_issue['total'])}, {top_issue['count']})" if top_issue else "—"
            ...
            riser_str = f"{biggest_riser['name']} (+{format_amount(biggest_riser['delta'])})" if biggest_riser and biggest_riser['delta'] > 0 else "—"
            ...
            largest_str = f"{largest['client_name']} → {largest['registrant_name']} ({format_amount(largest['amount'])})" if largest else "—"
