...
def create_web_server(slack_app) -> Flask:
    ...
    def handle_slash_command():
        ...
        try:
            ...
            if not slack_app.verify_slack_request(
                dict(request.headers), request.get_data(as_text=True)
            ):
        ...
        try:
            ...
            if success:
                return jsonify(
                    {
                        "status": "success",
                        "message": f"{digest_type.title()} digest sent",
                    }
                )
            else:
                return (
                    jsonify({"status": "error", "message": "Failed to send digest"}),
                    500,
                )
