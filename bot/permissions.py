...
class PermissionManager:
    ...
    def __init__(self, slack_token: Optional[str] = None):
        ...
        self._admin_cache: Dict[str, Set[str]] = (
            {}
        )  # channel_id -> set of admin user_ids
    ...
    def is_channel_admin(self, channel_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            if (
                channel_id in self._admin_cache
                and channel_id in self._last_cache_update
                and now - self._last_cache_update[channel_id] < self._cache_ttl
            ):
    ...
    def _fetch_channel_admins(self, channel_id: str) -> Set[str]:
        ...
        try:
            ...
            if not info_data.get("ok"):
                logger.error(
                    f"Slack API error getting channel info: {info_data.get('error')}"
                )
                return set()
            ...
            if not members_response.ok:
                logger.error(
                    f"Failed to get channel members: {members_response.status_code}"
                )
                return set()
            ...
            if not members_data.get("ok"):
                logger.error(
                    f"Slack API error getting members: {members_data.get('error')}"
                )
                return set()
