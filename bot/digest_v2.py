...
class DigestV2Formatter:
    ...
    def format_mini_digest(self, signals: List[SignalV2]) -> Optional[str]:
        ...
        high_priority = sorted(high_priority, key=lambda x: x.priority_score, reverse=True)[:3]
        ...
        lines.append(f"_{len(processed_signals)} signals in last 4h, {len(high_priority)} high-priority_")
    ...
    def _get_industry_snapshots(self, signals: List[SignalV2]) -> List[SignalV2]:
        ...
        for industry, industry_signals in industry_groups.items():
            industry_signals = sorted(industry_signals, key=lambda x: x.priority_score, reverse=True)
    ...
    def _get_docket_surge_signals(self, signals: List[SignalV2]) -> List[SignalV2]:
        ...
        return sorted(surge_signals, key=lambda x: x.metric_json.get('comments_24h_delta_pct', 0), reverse=True)
    ...
    def _should_send_mini_digest(self, signals: List[SignalV2]) -> bool:
        ...
        if any(s.signal_type == SignalType.DOCKET and 
               s.metric_json and 
               s.metric_json.get('comments_24h_delta_pct', 0) >= 200 
               for s in signals):
            return True
