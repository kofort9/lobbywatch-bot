...
class DailySignalsCollectorV2:
    ...
    def collect_all_signals(self, hours_back: int = 24) -> List[SignalV2]:
        ...
        try:
            ...
            logger.info(f"Collected {len(reg_gov_signals)} signals from Regulations.gov")
        except Exception as e:
    ...
    def _collect_congress_signals(self, hours_back: int) -> List[SignalV2]:
        ...
        # Get recent bills
        bills_url = f"https://api.congress.gov/v3/bill/118"
        params = {
        ...
        try:
            ...
            for bill in data.get('bills', []):
                ...
                actions_response = self.session.get(actions_url, params=params, timeout=30)
    ...
    def _create_congress_signal(self, bill: Dict[str, Any], action: Dict[str, Any]) -> Optional[SignalV2]:
    ...
    def _collect_regulations_gov_signals(self, hours_back: int) -> List[SignalV2]:
        ...
        try:
            response = self.session.get(dockets_url, params=params, headers=headers, timeout=30)
            response.raise_for_status()
            data = response.json()
    ...
    def _create_regulations_gov_signal(self, docket: Dict[str, Any]) -> Optional[SignalV2]:
    ...
    def _map_bill_to_issues(self, bill: Dict[str, Any], action: Dict[str, Any]) -> List[str]:
