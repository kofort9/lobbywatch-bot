...
class TestLDAFrontPageDigest(unittest.TestCase):
    ...
    def _create_test_data(self):
        ...
        with self.db_manager.get_connection() as conn:
            ...
            filings = [
                (1, 'filing-001', 11, 1, '2024-07-15', '2024Q3', 2024, 420000, 'original', False, '2024-09-29T10:00:00Z'),
                (2, 'filing-002', 12, 2, '2024-08-01', '2024Q3', 2024, 380000, 'original', False, '2024-09-29T11:00:00Z'),
                (3, 'filing-003', 11, 1, '2024-09-15', '2024Q3', 2024, 320000, 'amended', True, '2024-09-29T14:00:00Z'),
            ]
            
            for filing_data in filings:
                filing_id, filing_uid, client_id, registrant_id, filing_date, quarter, year, amount, filing_status, is_amendment, ingested_at = filing_data
                conn.execute("""
                ...
                """, (filing_id, filing_uid, client_id, registrant_id, filing_date, quarter, year, amount,
                      filing_status, is_amendment, ingested_at, filing_uid))
            
            # Create filing-issue relationships
            issue_map = {}
            cursor = conn.execute("SELECT id, code FROM issue WHERE code IN ('TEC', 'HCR')")
